...
@st.cache_resource
def load_model():
    model_path = os.path.join(os.path.dirname(__file__), 'bicep_model.h5')
...
class ExerciseProcessor(VideoProcessorBase):
    ...
    def calculate_angle(self, landmarks):
        ...
        shoulder = get_coordinates(landmarks[self.mp_pose.PoseLandmark.LEFT_SHOULDER.value])
        elbow = get_coordinates(landmarks[self.mp_pose.PoseLandmark.LEFT_ELBOW.value])
        wrist = get_coordinates(landmarks[self.mp_pose.PoseLandmark.LEFT_WRIST.value])
    ...
    def display_metrics(self, frame, metrics):
        if metrics:
            ...
            cv2.putText(frame, f"Elbow Angle: {int(metrics['elbow_angle'])}", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

class VideoProcessor(VideoProcessorBase):
    def __init__(self, exercise_processor):
    ...
    def recv(self, frame):
        ...
        # Process the frame
        processed_frame, metrics = self.exercise_processor.process_frame(img)
        
        # Display metrics on the frame
        cv2.putText(processed_frame, f"Reps: {metrics.get('reps', 0)}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(processed_frame, f"Form Accuracy: {metrics.get('form_accuracy', 0):.2f}%", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(processed_frame, f"Current Form: {'Correct' if metrics.get('correct_form', False) else 'Incorrect'}", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(processed_frame, f"Elbow Angle: {metrics.get('elbow_angle', 0):.2f}°", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    if video_option == "Live Webcam":
        ...
        if st.session_state.webcam_running:
            ...
            while st.session_state.webcam_running:
                ...
                # Display metrics on the frame
                cv2.putText(processed_frame, f'Reps: {metrics.get("reps", 0)}', (50, 100),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                cv2.putText(processed_frame, f'Form Accuracy: {metrics.get("form_accuracy", 0):.2f}%', (50, 150),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                cv2.putText(processed_frame, f'Elbow Angle: {int(metrics.get("elbow_angle", 0))}', (50, 200),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                cv2.putText(processed_frame, f'Form: {"Correct" if metrics.get("correct_form", False) else "Incorrect"}', (50, 250),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if metrics.get("correct_form", False) else (0, 0, 255), 2, cv2.LINE_AA)
